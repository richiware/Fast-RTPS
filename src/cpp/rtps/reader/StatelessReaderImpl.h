...
#ifndef __RTPS_READER_STATELESSREADERIMPL_H__
#define __RTPS_READER_STATELESSREADERIMPL_H__
 ...
 */
class StatelessReader::impl : public RTPSReader::impl
{
    public:

        impl(RTPSParticipant::impl&, const GUID_t& guid,
                const ReaderAttributes& att, ReaderHistory* hist, RTPSReader::impl::Listener* listener = nullptr);
...
inline StatelessReader* create_statelessreader_from_implementation(std::shared_ptr<RTPSReader::impl>& impl)
{
    assert(dynamic_cast<StatelessReader::impl*>(impl.get()) != nullptr);
    return new StatelessReader(impl);
}
...
#endif
#endif /* __RTPS_READER_STATELESSREADERIMPL_H__*/
